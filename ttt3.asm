...
recentCicle: dw 0x0         ; pos val 
recentCross: dw 0x0         ; pos val
recentCrossBox: dw 0x0      ; stores address of the box structure
recentCicleBox: dw 0x0      ; stores address of the box structure
tcircle: dw 0x0             ; total marked boxes on screen by player
tcross: dw 0x0              ; total marked boxes on screen by player
...
handle_input:
    ...
    not_occupied:
        ...
        cross_turn:
                ...
                push word [si]
                call draw_symbol
                mov byte [si+2], 0x1  ; for cross  === occupied

                mov bx, [si]
                mov word [recentCross], bx      ; save position
                mov word [recentCrossBox], si   ; save box address
                add word [tcross], 1 ; inc count of crossrs on screen
        ...
        circle_turn:
            ...
            jne exit_handle_input
                ...
                mov bx, [si]
                mov word [recentCicle], bx      ; save position
                mov word [recentCicleBox], si   ; save box address
                add word [tcircle], 1 
...
lim_exceed: 
    ...
    cmp byte [turn], 0x1
        jne circle_lim_check

        cross_lim_check: 
                ; check if 3 box filled
                cmp word [tcross], 3
                jne circle_lim_check

                    ; remove the oldest cross
                    mov si, word [recentCrossBox]   ; get the box address
                    cmp si, 0                       ; check if we have a recent box
                    je circle_lim_check

                    ; remove cross recemt ome
                            push white
                            push empty
                            push word [si]   ; recenr bpx 
                            call draw_symbol ; call Undraw_symbol
                            mov byte [si+2], 0x0   ; is occupied: null
                            sub word [tcross], 1   ; decrease count
                            jmp exit_lim
                ...
                ; check if 3 box filled
                        ...
                        jne exit_lim
                            ...
                            ; remove the oldest circle
                                mov si, word [recentCicleBox]   ; get the box address
                                cmp si, 0                       ; check if we have a recent box
                                je exit_lim
                            ...
                            ; remove cross recemt ome
                                    ...
                                    push empty
                                    push word [si]  ; recenr bpx 
                                    call draw_symbol ; Undraw_symbol 
                                    mov byte [si+2], 0x0   ; mark as unoccupied
                                    sub word [tcircle], 1   ; decrease count
...
check_equal_and_not_empty:
                                ; Stack k khoobsurat halaat: Res, Num , Num, Num, Ip, Bp
